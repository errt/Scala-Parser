...
class FtanAcceptorParser(acceptor: Acceptor) extends RegexParsers {
  ...
  def boolean =
    ("true" | "false") ^^ {
      value => acceptor.processBoolean(value.toBoolean); println(value)
    }

  def number =
    """-?(0|[1-9][0-9]*)(\.[0-9]+)?([eE][+-]?[0-9]+)?""".r ^^ {
      value => acceptor.processNumber(value.toDouble); println(value)
    }
  ...
  private def _string: Parser[String] = {
    ...
    (quoted_string('"') | quoted_string('\'')) ^^ {str=>println("Str");str}
  }
  ...
  def element: Parser[Unit] = {
    def attributes = {
      ...
      def name = (nameWithoutQuotes|_string) ^^ {name=>println("name");name}
      def pair = {
        def pairKey =  (name<~"=") ^^ {case name=>acceptor.processAttributeName(name)}
        pairKey ~ value ^^ {_=>println("test4")}
      }
      def firstpair =
        /*(pair | name) ^^ {
      ...
      {
      	var t = ""
      	def firstname = name ^^ {case name=>t=name}
      	def equals = "=" ^^ {_=>acceptor.processStartElement(None); println("test1"); acceptor.processAttributeName(t)}
        firstname ~> ((equals ~ value)?) ^^ {_.getOrElse(acceptor.processStartElement(Some(t)))}
      }
      firstpair <~ (pair*) ^^ {_=>println("test3")}
    }
    def content = {
    ...
    val startAndAttributes = ("<" ~> (attributes?)) ^^ {_.getOrElse{acceptor.processStartElement(None); println("test2")}}
    
    val contentStart = "|" ^^ {_=>acceptor.processStartContent(false)}

    startAndAttributes ~ ((contentStart ~> content)?) <~ ">" ^^ {_=> acceptor.processEndElement}
  }
